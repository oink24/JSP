		...
		formComment.onsubmit = async (e) => {
			...
			*/
			...
			if (data != null)
			{
				...
				alert('댓글이 등록되었습니다.');
				formComment.content.value = null;
			}
			else
			{
				alert('댓글 등록에 실패하였습니다. 다시 시도해보세요.');
			}
		};
		
		// 댓글 삭제 (동적 이벤트 바인딩 처리 -> 동적 생성되는 새로운 댓글목록 삭제링크가 동작함)
		document.addEventListener('click', function(e){
			...
			if (e.target && e.target.classList.value == 'remove')
			{
				...
				.then(data => {
					...
					{
						alert('댓글 삭제에 실패하였습니다. 다시 시도해보세요.');
					}
				});
			}
		});
			...
			if (e.target && e.target.classList.value == 'modify')
			{
				...
				else if (txt == '수정완료')
				{
					...
					if (data.result > 0)
					{
						...
						alert('댓글 수정에 실패하였습니다. 다시 시도해보세요.');
					}
				}
			}
			    ...
			    <section class="commentList">
			        ...
			        <article>
						...
						<div>
							...
							<a href="#" class="cancel" data-no="${comment.no}">취소</a> <!-- style.css display:none 처리 -->
