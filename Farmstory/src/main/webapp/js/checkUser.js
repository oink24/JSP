...
$(function(){
	...
	$('#btnCheckUid').click(function(){
		...
		$.ajax({
			url: '/Farmstory/user/checkUid.jsp',
			type: 'GET',
			data: jsonData,
			dataType: 'json',
	...
	$('input[name=nickname]').focusout(function(){
		...
		// 데이터 전송
		$.get('/Farmstory/user/checkNick.jsp', jsonData, function(data){
	...
	document.getElementsByName('email')[0].onfocusout = function(){
		...
		xhr.open('GET', '/Farmstory/user/checkEmail.jsp?email='+email);
		xhr.send();
	...
	document.getElementsByName('hp')[0].addEventListener('focusout', function(){
		...
		// 데이터 전송
		const url = '/Farmstory/user/checkHp.jsp?hp='+this.value;
